...
/**
 * Improved Mass Image Generator for Quaternion Attractor
 * 
 * This tool generates a large number of images with their corresponding parameter files
 * for easy regeneration and analysis. Each image gets a matching parameter file.
 */
...
class ImprovedMassImageGenerator {
  constructor() {
  ...
  /**
   * Generate a single image with parameter file
   */
  async generateImage(index, outputDir) {
    ...
    try {
      ...
      // Save parameter file with matching prefix
      const parameterFilename = this.saveParameterFile(parameters, filename, outputDir);
  ...
  /**
   * Generate mass collection of images with parameter files
   */
  async generateMassCollection(count = 100) {
    console.log(`🎲 Improved Mass Image Generator`);
    console.log(`================================`);
    console.log(`Generating ${count} random attractor images with parameter files...`);
    
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputDir = path.join(__dirname, '..', 'output', 'improved_mass_generation', timestamp);
    ...
    console.log(`\n🎉 Improved mass generation complete!`);
  ...
  async saveCollectionSummary(outputDir, requestedCount, successCount, startTime) {
    ...
    const summaryPath = path.join(outputDir, 'improved_mass_generation_summary.json');
 ...
 */
async function main() {
  const generator = new ImprovedMassImageGenerator();
  
  // Get count from command line argument or use default
  const count = process.argv[2] ? parseInt(process.argv[2]) : 100;
  
  console.log(`🚀 Starting improved mass image generation...`);
  console.log(`Target: ${count} images with parameter files for comprehensive dataset`);
  ...
  console.log(`2. Use parameter files to regenerate interesting images`);
  console.log(`3. Run AI analysis on the new images`);
  console.log(`4. Identify the most interesting parameter combinations`);
  console.log(`5. Use organized results for further research`);
}
...
module.exports = { ImprovedMassImageGenerator };
